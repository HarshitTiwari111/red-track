...
import NetworkModal, { blankNetwork, networkToForm } from '../components/NetworkModal.jsx';
import NetworkCatalogModal from '../components/NetworkCatalogModal.jsx';
...
const STORAGE = 'kap.networks.table';
...
function cellValue(row, key, { onEdit }) {
  switch (key) {
      ...
      return (
        <>
          ...
          <span className="cell-sub">
            ...
            {row.clickExpiration?.enabled ? `${row.clickExpiration.days}d window` : 'no expiry'}
          </span>
        </>
      );
    case 'status':
    ...
    case 'duplicateMode':
      return <span className="badge neutral">{row.duplicateMode || 'update'}</span>;
    case 'currency':
...
export default function Networks() {
  ...
  const [draft, setDraft] = useState({ from: todayKey(), to: todayKey(), title: '', status: 'all' });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await api.get('/networks/table', { params });
      setRows(data.rows);
      setTotals(data.totals);
      setSelected(new Set());
    } catch (err) {
      setError(errMsg(err, 'Could not load offer sources'));
    } finally {
      setLoading(false);
    }
  }, [filters]);
  ...
  const save = async () => {
    ...
    setFormError('');
    try {
      if (editing._id) await api.put(`/networks/${editing._id}`, editing);
      else await api.post('/networks', editing);
      setEditing(null);
      load();
    } catch (err) {
      setFormError(errMsg(err, 'Could not save the offer source'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const bulk = async (body, message) => {
    try {
      await api.post('/networks/bulk', { ids: selectedIds, ...body });
      setNotice(message);
  ...
  const clone = async () => {
    try {
      const { data } = await api.post(`/networks/${selectedIds[0]}/clone`);
      setNotice(`Cloned as "${data.name}" with a fresh security key — starts paused.`);
  ...
  const removeSelected = () => {
    if (!window.confirm(`Delete ${selectedIds.length} offer source(s)? Recorded conversions are kept.`)) return;
  ...
  const exportCsv = () => {
    ...
    a.href = url;
    a.download = `offer-sources-${filters.from}_${filters.to}.csv`;
    a.click();
    ...
    >
      ...
      <div className="action-bar" ref={menuRef}>
        ...
        <button
          ...
          className="action-btn"
          disabled={!one}
          onClick={() => navigate(`/conversions?networkId=${one._id}`)}
        >
        ...
        <div className="popover-wrap">
          ...
          {menu === 'status' && (
            <div className="popover">
              <label onClick={() => bulk({ action: 'status', status: 'active' }, 'Offer sources activated.')}>
                Activate
              </label>
              <label onClick={() => bulk({ action: 'status', status: 'paused' }, 'Offer sources paused.')}>Pause</label>
            </div>
          )}
        </div>
        ...
        <span className="mute" style={{ fontSize: 12, marginLeft: 'auto' }}>
          {selectedIds.length ? `${selectedIds.length} selected` : `${rows.length} offer sources`}
        </span>
      </div>
          ...
          <table className={`data density-${prefs.density}`}>
            ...
            <tbody>
              ...
              {!loading && visible.length === 0 && (
                <tr>
                  <td colSpan={columns.length + 1} className="table-empty">
                    No offer sources match these filters
                  </td>
                </tr>
              )}
              {!loading &&
      ...
      <div className="hint" style={{ marginTop: 10 }}>
        An offer source&apos;s numbers are the sum of the offers assigned to it. Double-click a row to edit, or use
        Report to open its conversions.
      </div>
